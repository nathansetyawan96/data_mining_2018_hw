...
def main():
    ...
    with open('/home/yccheng/project/Web/webcam/AQ_PTT.test7.0.txt',encoding = 'utf-8-sig') as f:
        #pred_result = f.readline()
        caption = f.readline()
    title = caption.split(']')[1]
    print(title)
    content = '冬天到,已經一陣子沒關心空氣品質,{},有沒有卦？'.format(caption.split(']')[0][1:])
    time.sleep(1)
    if ptt.is_connect():
        if ptt.login():
#            ptt.post('Gossiping', title, content)
            ptt.post('Test', title, content)
    ptt.logout()
